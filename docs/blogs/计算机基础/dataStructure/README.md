...
![shunxuASL](../../../.vuepress/public/assets/img/computerBasics/dataStructure/shunxuASL.png)
...
![zhebanASL](../../../.vuepress/public/assets/img/computerBasics/dataStructure/zhebanASL.png)
<br>
...
![B树](../../../.vuepress/public/assets/img/computerBasics/dataStructure/B.png)
<br>
...
B+树的特点：
![B+树](../../../.vuepress/public/assets/img/computerBasics/dataStructure/B+.png)
<br>
...
排序的分类：
1. 内部排序：在排序期间元素全部存放在**内存中**的排序
2. 外部排序：在排序期间元素**无法全部同时存放在内存中**
...
空间复杂度为O(1)
时间复杂度：
- 最好情况：元素已经有序，只需要比较而不用移动元素，**时间复杂度为O(n)**
- 最坏情况：元素逆序，总比较次数$$\sum_{i=2}^ni$$，总移动次数$$\sum_{i=2}^n(i+1)$$
- 平均时间复杂度为O($n^2$)

稳定性：**稳定**
适用性：适用于**顺序存储**和**链式存储的线性表**
...
| 空间复杂度 | 时间复杂度 | 稳定性 |
| :------: | :------: | :------: |
| O(1) | 最坏：O($n^2$)，平均：O($n^2$) | 稳定 |

冒泡排序中产生的有序子序列是**全局有序**的
...
快速排序的操作：首先从数列的右边开始往左边找，我们设这个下标为 i，也就是进行减减操作（i--），找到第 1 个比基准数小的值，让它与基准值交换；接着从左边开始往右边找，设这个下标为 j，然后执行加加操作（j++），找到第 1 个比基准数大的值，让它与基准值交换；然后继续寻找，直到 i 与 j 相遇时结束，最后基准值所在的位置即 k 的位置，也就是说 k 左边的值均比 k 上的值小，而 k 右边的值都比 k 上的值大。

快速排序算法的性能也主要取决于**划分操作的好坏**

<br/>

| 空间复杂度 | 时间复杂度 | 稳定性 |
| :------: | :------: | :------: |
| 需要递归，最好、平均：**O($\log_2n$)**, 最坏：**O(n)** | 与划分是否对称有关，最坏：**O($n^2$)**, 平均：**O($n\log_2n$)** | 不稳定 |

**快速排序是所有内部排序算法中平均性能最优的排序算法**

快速排序的特点：
1. **第i趟完成时，会有i个以上的树出现在它最终将要出现的位置，即它左边的数都比他小，右边的数都比他大**
2. 初始排序表**基本有序或基本逆序**时，达到最坏时间复杂度
...
1. 当且仅当满足其一：
   - L(i)>=L(2i)且L(i)>=L(2i+1)*（大根堆/大顶堆）*
   - L(i)<=L(2i)且L(i)<=L(2i+1)*（小根堆/小顶堆）*
2. 是一棵完全二叉树
...
**重要操作：**
- 插入：新节点先放在堆的末端，再对这个新节点向上执行调整操作
- 删除
- 输出对顶元素，将堆的最后一个元素与堆顶元素交换，继续维护堆


堆排序**适合关键字较多**的情况，例如：在一亿个数中选出前100个最大值
...
| 空间复杂度 | 时间复杂度 | 稳定性 |
| :------: | :------: | :------: |
| O(n) | **O($n\log_2n$)**。每趟归并O(n),共需$\lceil \log_2n \rceil$趟 | 稳定 |

一般而言，对于**N**个元素进行**k**路归并排序时，排序的趟数**m**满足**k$^m$=N**，从而**m=$log_k$N**，又考虑到m为整数，所以**m=$\lceil \log_kN \rceil$**

<br/>

#### 8.5.2 基数排序、

基数排序，是基于**关键字各位的大小**进行排序。是一种借助多关键字排序的思想对**单逻辑关键字**进行排序的方法
...
| 空间复杂度 | 时间复杂度 | 稳定性 |
| :------: | :------: | :------: |
| O(r),r个队列 | **O(d(n+r))**，d趟分配和收集，一趟收集需要O(r) | 稳定 |
...
#### 8.6.1 基本方法
外部排序常采用的排序方法是**归并排序**，这种归并方法由两个不同的阶段组成
...
只要增大归并路数k，或减少初始归并段个数r，都能减少归并趟数S
...
使用败者树之后，**内部归并的比较次数与k无关**。因此，**只要内存空间允许，增大归并路数k将有效减少归并树的高度，从而减少I/O次数，提高外部排序的速度**
...
步骤：
1. 首先从初始文件中输入 w 个记录到内存工作区中；
2. 从内存工作区中选出关键字最小的记录，将其记为 MINIMAX 记录；
3. 将 MINIMAX 记录输出到归并段文件中；
4. 此时内存工作区中还剩余 w-1 个记录，若初始文件不为空，则从初始文件中输入下一个记录到内存工作区1. 中；
...
最佳归并树在外部排序中的作用是：**设计m路归并排序的优化方案**

<br/><br/>

### 8.7 内部排序的比较及应用

在排序过程中，每趟都能确定一个元素在其最终位置的有**冒泡排序、简单选择排序、堆排序、快速排序**，其中前三者能形成全局有序的子序列，后者能确定枢轴元素的最终位置

<br/>

##### 各种内部排序算法的性质比较
![cmp](../../../.vuepress/public/assets/img/computerBasics/dataStructure/cmp.png)
<br>
...
- **n较小**：**直接插入排序或简单选择排序**。直接插入排序的**记录移动次数**比简单选择排序多，所以当记录本身信息量较大时，用简单选择排序好
- **n较大**：**快速排序、堆排序或归并排序**。
- 当**记录本身信息量较大**时，可用**链表**作为存储结构
